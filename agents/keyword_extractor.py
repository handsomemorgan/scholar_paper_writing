...
为每个一级关键词建议一个检索组合（关键词布尔表达式），例如：
- "(深度学习 OR deep learning) AND (图像识别 OR image recognition)"
...
请严格输出以下JSON格式：
{
    ...
    "search_queries": [
        {
            "query": "(deep learning OR 深度学习) AND (image recognition OR 图像识别)",
...
class KeywordExtractor:
    ...
    def _fallback_extract(self, requirements: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "tertiary_keywords": [],
            "search_queries": [
                {"query": kw["zh"], "source": "google_scholar", "priority": "high"}
                for kw in primary
            ],
